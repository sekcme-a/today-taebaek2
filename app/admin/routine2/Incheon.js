...
export default function Incheon({
  ...
  const fetchArticles = async () => {
    ...
    setLog((prev) => [
      ...prev,
      `🚀 인천 크롤링 시작: 총 ${dateList.length}일치 데이터 요청 중...`,
    ]);
      ...
      const requests = dateList.map(async (date) => {
        try {
          const res = await fetch(
            `/api/crawl/incheon?start=${date}&end=${date}&page=0`,
          );
          ...
          const cleaned = (data?.articles ?? []).map((article) => ({
            ...article,
            title: article.title.startsWith("인천시, ")
              ? article.title.replace("인천시, ", "")
              : article.title,
          }));
  ...
  const saveArticles = async (e) => {
    try {
      ...
      // 4. 카테고리 매핑 데이터 생성
      // (기존 slugList, general, incheon 리스트 생성)
      ...
      const incheonSlugList = insertedArticles.map((item) => ({
        article_id: item.id,
        category_slug: "incheon",
      }));

      // 5. article_categories 테이블 insert
      const { error: categoryError } = await supabase
        .from("article_categories")
        .insert([...slugList, ...generalSlugList, ...incheonSlugList]);
      ...
      setHistory((prev) => [
        ...
        {
          type: "success",
          message: `인천 보도자료 ${datas.length}개 저장 성공`,
      ...
      setHistory((prev) => [
        ...
        {
          type: "error",
          message: `인천 보도자료 저장 실패: ${error.message}`,
          articleIds: [],
        },
      ]);
    }
